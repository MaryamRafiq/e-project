...
@section('content')
    <h1 class="text-center text-5xl" style="margin-top: 5rem; color:rgb(255, 255, 255); font-weight:bold">ALL REPORTS</h1>
            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                    <div class="w-full md:w-1/2">
                        <form action="{{ route('reports.index') }}" method="GET" class="flex items-center space-x-4">
                    ...
                    <div
                        ...
                        <button type="button"
                            ...
                            <a href="/reports/create" class="text-white hover:text-white">Generate Report</a>
                        </button>
                    </div>
                </div>
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                <th scope="col" class="px-4 py-3">Testing ID</th>
                        ...
                        <tbody>
                            @forelse ($reports as $report)
                                <tr class="border-b dark:border-gray-700">
                                    ...
                                    <td class="px-4 py-3 max-w-[12rem] truncate">{{ $report->content }}</td>
                                    <td class="px-4 py-3">{{ $report->status }}</td>
                                    <td class="px-4 py-3">{{ $report->user_id }}</td>
                                    <td class="px-4 py-3">{{ $report->testing_id }}</td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-4 py-3 text-center">No reports found</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
