...
@section('content')
    <h1 class="text-center text-5xl" style="margin-top: 5rem; color:rgb(255, 255, 255); font-weight:bold">ALL TESTERS</h1>
            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                    <div class="w-full md:w-1/2">
                        <form action="{{ route('testings.index') }}" method="GET" class="flex items-center space-x-4">
                    ...
                    <div
                        ...
                        <button type="button"
                            ...
                            <a href="/testings/create" class="text-white hover:text-white">Add Tester</a>
                        </button>
                    </div>
                </div>
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                <th scope="col" class="px-4 py-3">Tester Role</th>
                        ...
                        <tbody>
                            @forelse ($testings as $testing)
                                <tr class="border-b dark:border-gray-700">
                                    <td class="px-4 py-3">{{ $testing->id }}</td>
                                    <td class="px-4 py-3">{{ $testing->name }}</td>
                                    <td class="px-4 py-3 max-w-[12rem] truncate">{{ $testing->description }}</td>
                                    <td class="px-4 py-3">{{ $testing->role }}</td>
                                    <td class="px-4 py-3">{{ $testing->status }}</td>
                                    <td class="px-4 py-3 flex items-center justify-end">
                                        ...
                                        <button type="button"
                                            ...
                                            <a href="/testings/{{ $testing->id }}/delete">
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-4 py-3 text-center">No testers found</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
